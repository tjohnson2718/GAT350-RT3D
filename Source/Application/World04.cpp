...
namespace nc
{
    bool World04::Initialize()
    {
        ...
        for (int i = 0; i < 3; i++) 
        {
            m_lights[0].type = light_t::eType::Point;
            m_lights[0].position = glm::vec3{ randomf(-5, 5), randomf(1, 8), randomf(-3, 6) };
            m_lights[0].direction = glm::vec3{ 0, -1, 0 };
            m_lights[0].color = glm::rgbColor(glm::vec3{ randomf() * 360, 1, 1 });
            m_lights[0].intensity = 1;
            m_lights[0].range = 5;
            m_lights[0].innerAngle = 10.0f;
            m_lights[0].outerAngle = 30.0f;
        }
